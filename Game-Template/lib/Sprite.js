...
export default class Sprite {
	 ...
	 * @returns
	 */
	static generateSpritesFromSpriteSheet(spriteSheet, tileWidth, tileHeight, xOffset = 0, yOffset = 0) {
		tileWidth += xOffset;
		tileHeight += yOffset;
		const sprites = [];
		...
		for (let y = 0; y < sheetHeight; y++) {
			for (let x = 0; x < sheetWidth; x++) {
				sprites.push(new Sprite(spriteSheet, xOffset + x * tileWidth, yOffset+ y * tileHeight, tileWidth - xOffset, tileHeight - yOffset));
			}
		}
